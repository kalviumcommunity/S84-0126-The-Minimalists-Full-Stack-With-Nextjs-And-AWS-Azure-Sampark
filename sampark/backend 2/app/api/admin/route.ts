...
import { getStatusUpdateTemplate } from '../../../lib/email-templates.js';
...
router.patch("/grievances/:id/status", verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    if (updatedGrievance && updatedGrievance.user) {
      const oldStatus = grievance.statuses[0]?.status;
      const emailTemplate = getStatusUpdateTemplate(updatedGrievance.user.name, {
        trackingId: updatedGrievance.trackingId,
      ...
      sendEmail(updatedGrievance.user.email, emailTemplate.subject, emailTemplate.text)
        .then((success) => {
          if (success) {
            console.log(`✅ Status update email sent to ${updatedGrievance.user.email}`);
          }
        })
        .catch((error) => {
          console.error(`❌ Failed to send status update email to ${updatedGrievance.user.email}:`, error);
        });
    }
