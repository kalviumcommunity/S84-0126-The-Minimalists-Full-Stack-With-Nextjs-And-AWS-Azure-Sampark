...
import { getGrievanceConfirmationTemplate } from '../../../lib/email-templates.js';
...
router.post("/submit", verifyToken, async (req, res) => {
  try {
    ...
    const emailTemplate = getGrievanceConfirmationTemplate(grievance.user.name, {
    ...
    sendEmail(grievance.user.email, emailTemplate.subject, emailTemplate.text)
      .then((success) => {
        if (success) {
          console.log(`✅ Grievance confirmation email sent to ${grievance.user.email}`);
        }
      })
      .catch((error) => {
        console.error(`❌ Failed to send confirmation email to ${grievance.user.email}:`, error);
      });
